...
output "ssh_bastion_command" {
  ...
  value       = "ssh -i /path/to/key.pem ubuntu@${module.bastion.public_ip}"
}

output "ssh_jenkins_via_bastion" {
  description = "SSH command to connect to Jenkins via Bastion"
  value       = "ssh -i /path/to/key.pem -J ubuntu@${module.bastion.public_ip} ubuntu@${module.jenkins.private_ip}"
}

output "ssh_sonarqube_via_bastion" {
  description = "SSH command to connect to SonarQube via Bastion"
  value       = "ssh -i /path/to/key.pem -J ubuntu@${module.bastion.public_ip} ubuntu@${module.sonarqube.private_ip}"
}

output "jenkins_port_forward_command" {
  description = "Command to port forward Jenkins UI (9000) through Bastion"
  value       = "ssh -i /path/to/key.pem -L 8080:${module.jenkins.private_ip}:8080 -J ubuntu@${module.bastion.public_ip} ubuntu@${module.jenkins.private_ip}"
}

output "sonarqube_port_forward_command" {
  description = "Command to port forward SonarQube UI (9000) through Bastion"
  value       = "ssh -i /path/to/key.pem -L 9000:${module.sonarqube.private_ip}:9000 -J ubuntu@${module.bastion.public_ip} ubuntu@${module.sonarqube.private_ip}"
}
